...
from app.helpers.point_systems import compute_points
...
class MatchesSchedule(db.Model):
    ...
    def sum_points(self):
        ...
        return compute_points(matches=matches,
    ...
    def rounds_points(self):
        ...
        for round in self.rounds:
            matches.extend(round.matches1vs1)
            this_round = compute_points(matches=round.matches1vs1,
                                        players=self.tournament.players,
                                        point_system=self.point_system,
                                        with_score=True)
            sum_round = compute_points(matches=matches,
                                       players=self.tournament.players,
                                       point_system=self.point_system,
                                       with_score=True)
            summary_round = []
            for this_player, this_score in this_round:
                for sum_player, sum_score in sum_round:
                    if this_player == sum_player:
                        summary_round.append((this_player, this_score, sum_score))
                        break

            rounds_points.append(summary_round)
    ...
    def next_round(self):
        ...
        sorted_players = compute_points(matches=matches,
                                        players=tournament.players,
                                        point_system=self.point_system,
                                        with_score=False)
